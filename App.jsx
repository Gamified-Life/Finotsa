...
const PulseTab = () => {
  ...
  const detailedCategories = [
    { emoji: '🍔', name: 'Food', amount: 6800, pct: 85, warning: true, shops: [
      { name: 'Swiggy', amount: 3200, date: 'Today' },
      { name: 'Zomato', amount: 2400, date: 'Yesterday' },
      { name: 'McDonalds', amount: 1200, date: '2 days ago' }
    ]},
    { emoji: '🚗', name: 'Transport', amount: 2400, pct: 45, warning: false, shops: [
      { name: 'Uber', amount: 1800, date: 'Multiple' },
      { name: 'Ola', amount: 600, date: 'Multiple' }
    ]},
    { emoji: '🎬', name: 'Subs', amount: 2849, pct: 95, warning: true, shops: [
      { name: 'Netflix', amount: 649, date: '12th May' },
      { name: 'Spotify', amount: 119, date: '14th May' },
      { name: 'Amazon Prime', amount: 2081, date: 'Annual' }
    ]},
    { emoji: '🛍️', name: 'Shopping', amount: 1200, pct: 30, warning: false, shops: [
      { name: 'Myntra', amount: 1200, date: 'Last week' }
    ]},
    { emoji: '☕', name: 'Café', amount: 890, pct: 60, warning: false, shops: [
      { name: 'Starbucks', amount: 550, date: 'Today' },
      { name: 'Blue Tokai', amount: 340, date: '3 days ago' }
    ]},
  ];
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      <header style={{ padding: '28px 24px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <p style={{ fontSize: 12, color: '#9CA3AF', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>Good evening, Rahul</p>
...
const CoachTab = () => {
  ...
  const opps = [
    { emoji: '🎬', title: 'Cancel Netflix', sub: 'Unused for 3 weeks', save: '₹649/mo', effort: 'Easy' },
    { emoji: '🍱', title: 'Cook 3x vs Swiggy', sub: 'Cut food variance', save: '₹1,200/mo', effort: 'Medium' },
  ];

  const taxOpps = [
    { title: 'ELSS Mutual Funds', desc: 'Invest ₹50,000 to max out 80C limit', save: 'Save ₹15,000 in tax', icon: '📈' },
    { title: 'Health Insurance', desc: 'Premium for parents under 80D', save: 'Save ₹7,500 in tax', icon: '🏥' },
  ];
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      <div style={{ padding: '0 24px 24px', textAlign: 'center' }}>
        <div style={{ position: 'relative', width: 160, height: 80, margin: '0 auto 16px' }}>
          <AnimatedArc progress={0.68} color={AMBER} strokeWidth={8} size={160} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, textAlign: 'center' }}>
            <span style={{ fontSize: 38, fontWeight: 800, color: AMBER, fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>68</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {opps.map((opp, i) => (
                    <div key={i} style={{ background: 'rgba(255,255,255,0.6)', borderRadius: 12, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <button style={{ background: AMBER, color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 100, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Act</button>
                    </div>
                  ))}
                </div>
              </div>
...
const EngineTab = () => {
  ...
  const rules = [
    { icon: <Zap size={15} color={INDIGO} />, name: 'Round-Up Invest', desc: 'Every UPI spend rounded to next ₹10, swept to Nifty 50', saved: '₹1,240', on: true },
    { icon: <Target size={15} color={INDIGO} />, name: 'Goal Auto-Transfer', desc: '₹500/week silently moved to Goa Trip fund', saved: '₹2,000', on: true },
    { icon: <Shield size={15} color={INDIGO} />, name: 'Subscription Shield', desc: 'Flags any new auto-debit before it hits', saved: '₹649 blocked', on: true },
  ];
        ...
        <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 20, padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <div>
              ...
              <AnimatedNumber value={4250} prefix="₹" duration={1200} style={{ fontSize: 26, fontWeight: 800, color: INDIGO, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.02em' }} />
            </div>
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 600, marginBottom: 2 }}>Returns</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: '#16A34A', fontVariantNumeric: 'tabular-nums' }}>+₹102</span>
                <span style={{ background: '#D1FAE5', color: '#16A34A', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 100 }}>+2.4%</span>
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  const tabs = [
    { id: 'home', label: 'Pulse', icon: Activity, accent: '#1A4731' },
    { id: 'coach', label: 'Coach', icon: Play, accent: '#B45309' },
    { id: 'engine', label: 'Engine', icon: Cpu, accent: '#4338CA' },
  ];
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh' }}>
        {tab === 'home' && <PulseTab key="home" />}
        {tab === 'coach' && <CoachTab key="coach" />}
        {tab === 'engine' && <EngineTab key="engine" />}
      </div>
